...
public final class ClasspathStoryFinder {
	private static final Logger LOG = LoggerFactory
			.getLogger(ClasspathStoryFinder.class);

	public static List<String> findFilenamesThatMatch(
			String aFilenameWithWildcards) {
	...
	private static Collection<File> findFilesThatMatch(
			String aFilenameWithWildcards) {
		WildcardFileFilter regexFileFilter = new WildcardFileFilter(
				aFilenameWithWildcards);
		List<File> rootDirsToSearchFrom = getRootDirs();
		LOG.debug("Searching for stories called {} in {}",
				aFilenameWithWildcards, rootDirsToSearchFrom);

		List<File> ret = new ArrayList<File>();
		for (File f : rootDirsToSearchFrom) {
			ret.addAll(listFiles(f, regexFileFilter,
	...
	private static List<File> getRootDirs() {
		List<File> ret = new ArrayList<File>();
		try {
			Enumeration<URL> roots = ClasspathStoryFinder.class
					.getClassLoader().getResources("");
			while (roots.hasMoreElements()) {
				File folderToSearch = new File(roots.nextElement().getFile());
				if (folderToSearch.isDirectory()) {
					ret.add(folderToSearch);
				}
